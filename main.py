...
def load_exam_data():
    return {
        ...
        "IC-39": {"title": "Fraud Risk Management", "credits": 30, "level": "Optional", "compulsory": False},
...
def load_user_data(username):
    if os.path.exists(USER_DATA_FILE):
        try:
            ...
            pass
    return {'passed_exams': {}, 'profile': {'name': username, 'created_at': datetime.now().strftime("%Y-%m-%d")}}
        ...
        if action == 'add_exam':
            ...
            if exam_code and exam_code in exam_data:
                if exam_code not in user_data['passed_exams']:
                    user_data['passed_exams'][exam_code] = {'passed': True, 'date': datetime.now().strftime("%Y-%m-%d")}
                    save_user_data(username, user_data)
            ...
            profile_fields = ['full_name', 'email', 'phone', 'registration_id', 'associated_institute', 
                            'membership_type', 'employer', 'designation', 'experience_years', 
                            'department', 'qualification', 'certifications']
            for field in profile_fields:
                if field in request.form:
                    user_data['profile'][field] = request.form.get(field, '')
            save_user_data(username, user_data)
...
def exams():
    ...
    for code, exam in exam_data.items():
        ...
        exams_by_level[exam['level']].append({'code': code, 'title': exam['title'], 'credits': exam['credits'], 'compulsory': exam['compulsory'], 'passed': is_passed})
                ...
                for username, data in all_users.items():
                    if data.get('password'):
                        ...
                        leaderboard_data.append({
                            'name': data.get('profile', {}).get('full_name', username),
                            'credits': credits,
                            'status': status,
...
def api_rank():
    ...
    exam_data = load_exam_data()
    user_data = load_user_data(username)
    user_credits = calculate_credits(user_data['passed_exams'], exam_data)
        ...
        try:
            with open(USER_DATA_FILE, 'r') as f:
                ...
                for u, data in all_users.items():
                    if data.get('password'):
                        credits = calculate_credits(data.get('passed_exams', {}), exam_data)
                        all_credits.append(credits)
        except:
            pass
    
    all_credits.sort(reverse=True)
    rank = all_credits.index(user_credits) + 1 if user_credits in all_credits else len(all_credits)
